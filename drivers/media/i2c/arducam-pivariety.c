...
static struct pivariety *to_pivariety(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct pivariety, subdev);
}
...
static u32 pivariety_get_format_code(struct pivariety *priv,
				     struct arducam_format *format)
{
	...
	order = (priv->hflip && priv->hflip->val ? order ^ 1 : order);
	order = (priv->vflip && priv->vflip->val ? order ^ 2 : order);

	v4l2_dbg(1, debug, &priv->subdev, "%s: before: %d, after: %d.\n",
		 __func__, origin_order, order);

	return data_type_to_mbus_code(format->data_type, order);
}

static int pivariety_s_power(struct v4l2_subdev *sd, int on)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct pivariety *priv = to_pivariety(client);
	int ret;

	if (on) {
		if (ret = regulator_bulk_enable(ARDUCAM_NUM_SUPPLIES,
			priv->supplies)) {
		...
		if (ret = clk_prepare_enable(priv->xlk)) {
...
reg_off:
		...
		regulator_bulk_disable(ARDUCAM_NUM_SUPPLIES, priv->supplies);
...
{
	struct pivariety *priv =
		container_of(ctrl->handler, struct pivariety, ctrl_handler);
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	i = pivariety_get_fmt_idx_by_code(pivariety, fmt->format.code);
	if (i < 0)
		i = 0;

	fmt->format.code = supported_formats[i].mbus_code;
	...
	fmt->format.width = supported_formats[i].resolution_set[0].width;
	fmt->format.height = supported_formats[i].resolution_set[0].height;
...
static int pivariety_probe(struct i2c_client *client)
{
	...
	priv = devm_kzalloc(&client->dev, sizeof(struct pivariety), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
	...
	v4l2_i2c_subdev_init(&priv->subdev, client, &pivariety_subdev_ops);
...
static const struct of_device_id pivariety_of_match[] = {
	{ .compatible = "arducam,pivariety" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, pivariety_of_match);

MODULE_DEVICE_TABLE(i2c, pivariety_id);
static struct i2c_driver pivariety_i2c_driver = {
	.driver = {
		.of_match_table	= of_match_ptr(pivariety_of_match),
		.name = PIVARIETY_NAME,
		.pm = &pivariety_pm_ops,
	},
	.probe = pivariety_probe,
	.remove = pivariety_remove,
	.id_table = pivariety_id,
};

module_i2c_driver(pivariety_i2c_driver);
MODULE_DESCRIPTION("Arducam Pivariety Camera driver");
