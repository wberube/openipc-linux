...
int camdriver_gpio_drv_set(struct gpio_chip *chip, unsigned offset, int value)
{
#ifdef CONFIG_PM_SLEEP
    gpio_state_table[offset].drv = value;
#endif
    return MDrv_GPIO_Drv_Set(offset, value);
}

int camdriver_gpio_pull_up(struct gpio_chip *chip, unsigned offset)
{
#ifdef CONFIG_PM_SLEEP
    gpio_state_table[offset].pull = GPIO_UP;
#endif
    return MDrv_GPIO_Pull_Up(offset);
}

int camdriver_gpio_pull_down(struct gpio_chip *chip, unsigned offset)
{
#ifdef CONFIG_PM_SLEEP
    gpio_state_table[offset].pull = GPIO_DOWN;
#endif
    return MDrv_GPIO_Pull_Down(offset);
}
...
static struct gpio_chip camdriver_gpio_chip = {
    ...
    .get_direction    = camdriver_gpio_get_direction,
    .base             = 0,
};
...
static int camdriver_gpio_resume(struct device *dev)
{
    ...
    for (i = 0; i < GPIO_NR; i++)
    {
        if (gpio_state_table[i].isreq == TRUE)
        {
            ...
            MDrv_GPIO_Drv_Set(i, gpio_state_table[i].drv);
