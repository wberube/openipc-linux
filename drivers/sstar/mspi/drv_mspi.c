...
static int sstar_spi_setup(struct spi_device *spi)
{
    ...
    if (sstar_spimst->mode != spi->mode)
    {
        ...
        HAL_MSPI_ChipSelect(&sstar_spimst->hal, ((spi->mode & SPI_CS_HIGH) == SPI_CS_HIGH), spi->chip_select);
...
static int sstar_spi_start_transfer(struct spi_device *spi, struct spi_transfer *tfr)
{
    int                err          = 0;
    struct sstar_mspi *sstar_spimst = spi_master_get_devdata(spi->master);
    ...
    */
    ...
    {
        HAL_MSPI_FullDuplex(spi->master->bus_num, &sstar_spimst->hal, (u8 *)sstar_spimst->rx_buf,
    ...
    else if (sstar_spimst->tx_buf != NULL)
    {
        ...
        {
            err = HAL_MSPI_DMA_Write(spi->master->bus_num, &sstar_spimst->hal, (u8 *)sstar_spimst->tx_buf,
        ...
        {
            err = HAL_MSPI_Write(spi->master->bus_num, &sstar_spimst->hal, (u8 *)sstar_spimst->tx_buf,
                                 (u32)sstar_spimst->len);
            if (err)
            {
                return -EIO;
    ...
    else if (sstar_spimst->rx_buf != NULL)
    {
        ...
        if (sstar_spimst->xfer_dma)
        {
            err = HAL_MSPI_DMA_Read(spi->master->bus_num, &sstar_spimst->hal, (u8 *)sstar_spimst->rx_buf,
                                    (u32)sstar_spimst->len);
            if (err)
            {
                return -EIO;
        ...
        {
            err = HAL_MSPI_Read(spi->master->bus_num, &sstar_spimst->hal, (u8 *)sstar_spimst->rx_buf,
                                (u32)sstar_spimst->len);
            if (err)
            {
                return -EIO;
...
static int sstar_spi_finish_transfer(struct spi_device *spi, struct spi_transfer *tfr, bool cs_change)
{
    struct sstar_mspi *sstar_spimst = spi_master_get_devdata(spi->master);
...
static int sstar_spi_transfer_one(struct spi_master *master, struct spi_message *mesg)
{
    int                  err = 0;
    bool                 cs_change;
...
#else
    struct clk *mspi_clk = of_clk_get(master->dev.of_node, 0);
    clk_prepare_enable(mspi_clk);
#endif
...
#else
    clk_disable_unprepare(mspi_clk);
    clk_put(mspi_clk);
#endif
...
static int sstar_spi_probe(struct platform_device *pdev)
{
    ...
    {
#ifdef CONFIG_MS_GPIO
        puse     = mdrv_padmux_getpuse(PUSE_SPI, mspi_group, SPI_DI);
        mosi_pad = mdrv_padmux_getpad(puse);
        ...
        {
            mosi_mode = mdrv_padmux_getmode(puse);
    ...
    mspi_base = (IO_ADDRESS(mspi_resource->start));
    mspi_dbgmsg("mspi_base = %#llx\n", mspi_base);
    ...
    sstar_spimst->hal.mspi_base    = (u64)mspi_base;
    sstar_spimst->use_dma          = use_dma;
    sstar_spimst->bus_num          = mspi_group;
    sstar_spimst->mosi_pad         = mosi_pad;
    sstar_spimst->mosi_mode        = mosi_mode;
    sstar_spimst->_4to3mode        = _4to3mode;
    ...
    if (!snprintf(sstar_spimst->irq_name, sizeof(sstar_spimst->irq_name), "mspi%d_Isr", mspi_group))
    {
        mspi_errmsg("find irq reformat failed\n");
        return -ENOENT;
    }
    mspi_dbgmsg("irq name : %s\n", sstar_spimst->irq_name);
    ...
    err = request_irq(irq_num, sstar_spi_interrupt, 0, sstar_spimst->irq_name, (void *)sstar_master);
    if (err == 0)
    {
    ...
    {
        mspi_errmsg("%s register failed", sstar_spimst->irq_name);
        goto err_irq;
    }
    ...
    sstar_spimst->mspi_clk_tbl.clk_src_tbl =
        kmalloc(sizeof(u32) * sstar_spimst->mspi_clk_tbl.clk_src_tbl_sz, GFP_KERNEL);
    ...
    sstar_spimst->mspi_clk_tbl.clk_div_tbl =
        kmalloc(sizeof(u32) * sstar_spimst->mspi_clk_tbl.clk_div_tbl_sz, GFP_KERNEL);
    ...
    sstar_spimst->mspi_clk_tbl.clk_cfg_tbl_sz =
        sstar_spimst->mspi_clk_tbl.clk_div_tbl_sz * sstar_spimst->mspi_clk_tbl.clk_src_tbl_sz;
    sstar_spimst->mspi_clk_tbl.clk_cfg_tbl =
        kmalloc(sizeof(struct mspi_clk) * sstar_spimst->mspi_clk_tbl.clk_cfg_tbl_sz, GFP_KERNEL);
    ...
    sstar_master->max_speed_hz =
        sstar_spimst->mspi_clk_tbl.clk_cfg_tbl[sstar_spimst->mspi_clk_tbl.clk_cfg_tbl_sz - 1].clk_rate;
...
static int sstar_spi_resume(struct device *dev)
{
    ...
    mspi_dbgmsg("resume mode:0x%x speed:%d channel:%d\n", sstar_spimst->mode, sstar_spimst->max_speed_hz,
                sstar_master->bus_num);
    return err;
}
