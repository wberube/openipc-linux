...
static u8 drv_spinand_write_after_read(u8 *pu8_wbuf, u32 u32_wsize, u8 *pu8_rbuf, u32 u32_rsize)
{
    u8 u8_wr_size;
    u8 u8_rd_size;
...
static u8 drv_spinand_simple_transmission(u8 *pu8_buf, u32 u32_size)
{
    u8 u8_wr_size;
...
static u8 drv_spinand_complete_transmission(u8 *pu8_buf, u32 u32_size)
{
    u8 u8_status;
...
static u8 drv_spinand_complete_get_features(u8 u8_command, u8 u8_address, u8 *pu8_data, u8 u8_size)
{
    ...
    if (!drv_spinand_write_after_read((u8 *)&u16_cmd, 2, pu8_data, (u32)u8_size))
    {
...
static u8 drv_spinand_complete_read_id(u8 *pu8_ids, u8 u8_bytes)
{
    ...
    if (!drv_spinand_write_after_read((u8 *)&u16_cmd, 2, pu8_ids, u8_bytes))
    {
        u8_status = ERR_SPINAND_TIMEOUT;
    }
...
static u8 drv_spinand_complete_read_from_cache_by_riu(u16 u16_col_address, u8 *pu8_data, u32 u32_size)
{
    ...
    do
    {
        ...
        u8_rsize = drv_spinand_write_after_read(au8_cmd, 4, pu8_data, u32_size);
...
#if defined(CONFIG_FLASH_BDMA)
static u8 drv_spinand_program_load_data_by_bdma(u8 *pu8_data, u32 u32_size)
{
    u8  u8_status = ERR_SPINAND_SUCCESS;
    u32 u32_write_bytes;
...
static u8 drv_spinand_load_data_to_cache(u8 *pu8_data, u32 u32_size)
{
    u8            u8_status = ERR_SPINAND_SUCCESS;
    ...
    while (0 != u32_size)
    {
        ...
        {
            u8_status = drv_spinand_simple_transmission(pu8_data, u32_bytes_to_load);
        }
        else
        {
            u8_status = _drv_spinand_program_load_data(pu8_data, u32_bytes_to_load);
        }
...
static u8 drv_spinand_complete_program_load(u8 u8_cmd, u16 u16_col_address, u8 *pu8_data, u32 u32_size)
{
    ...
    if (ERR_SPINAND_SUCCESS == (u8_status = drv_spinand_simple_transmission((u8 *)&u32_cmd, 3)))
    {
        ...
        u8_status = drv_spinand_load_data_to_cache(pu8_data, u32_size);
        DRV_FSP_set_access_mode(FSP_SINGLE_MODE);
    }
    DRV_QSPI_pull_cs(1);
...
static u8 drv_spinand_next_page_read(FLASH_CMD_SET_t *next)
{
    ...
    if (ERR_SPINAND_SUCCESS != drv_spinand_complete_transmission(au8_cmd, u8_i))
    {
        return ERR_SPINAND_DEVICE_FAILURE;
    }
...
static u8 drv_spinand_die_sel(u32 u32_page)
{
    ...
    if (g_pst_spinand_ext_info && (g_pst_spinand_ext_info->st_profile.u16_flags & SPINAND_MULTI_DIES))
    {
        ...
        if (ERR_SPINAND_SUCCESS != drv_spinand_complete_transmission(au8_cmd, 2))
        {
            return ERR_SPINAND_DEVICE_FAILURE;
        }
    }
