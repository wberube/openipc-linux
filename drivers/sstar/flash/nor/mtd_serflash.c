    ...
    *retlen = len;
    while (0 != len)
    {
        ...
        if (ERR_SPINOR_SUCCESS
            != (u8_status = mdrv_spinor_read(from, (u8 *)(_BDMA_ALLOC_DMEM.bdma_vir_addr), u32_read_size)))
        {
            *retlen = 0;
            mutex_unlock(&flash->lock);
            return (-EIO);
        }
        memcpy((void *)buf, (const void *)_BDMA_ALLOC_DMEM.bdma_vir_addr, u32_read_size);
        len -= u32_read_size;
        buf += u32_read_size;
        from += u32_read_size;
    }
