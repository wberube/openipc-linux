...
static u8 (*drv_spinor_program_load_data)(u8 *, u32);
...
static u8 drv_spinor_write_after_read(u8 *pu8_wbuf, u32 u32_wsize, u8 *pu8_rbuf, u32 u32_rsize)
{
    u8 u8_wr_size;
    u8 u8_rd_size;
...
static u8 drv_spinor_simple_transmission(u8 *pu8_buf, u32 u32_size)
{
    u8 u8_wr_size;
...
static u8 drv_spinor_complete_transmission(u8 *pu8_buf, u32 u32_size)
{
    u8 u8_status;

    DRV_QSPI_pull_cs(0);
    u8_status = drv_spinor_simple_transmission(pu8_buf, u32_size);
    DRV_QSPI_pull_cs(1);
...
static u8 drv_spinor_complete_read_status(u8 u8_cmd, u8 *pu8_status, u8 u8_size)
{
    ...
    if (!drv_spinor_write_after_read(&u8_cmd, 1, pu8_status, u8_size))
    {
...
static u8 drv_spinor_complete_write_status(u8 u8_cmd, u8 *pu8_status, u8 u8_size)
{
    ...
    DRV_QSPI_pull_cs(0);
    if (ERR_SPINOR_SUCCESS == (u8_status = drv_spinor_simple_transmission(&u8_cmd, 1)))
    {
        u8_status = drv_spinor_simple_transmission(pu8_status, u8_size);
    }
    DRV_QSPI_pull_cs(1);
...
static u8 drv_spinor_program_load_data_by_bdma(u8 *pu8_data, u32 u32_size)
{
    u8 u8_status = ERR_SPINOR_SUCCESS;
...
static u8 drv_spinor_complete_program_page(u8 u8_cmd, u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    do
    {
        ...
        if (ERR_SPINOR_SUCCESS != (u8_status = drv_spinor_simple_transmission(au8_cmd, 3)))
        {
            break;
        }
        ...
        while (0 != u32_size)
        {
            ...
            {
                u8_status = drv_spinor_simple_transmission(pu8_data, u32_bytes_to_load);
            }
            else
            {
                u8_status = drv_spinor_program_load_data(pu8_data, u32_bytes_to_load);
            }
...
static u8 drv_spinor_program_page(u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    if (0 != (u32_address & ~SPI_NOR_16MB_MASK))
    {
        ...
        if (ERR_SPINOR_SUCCESS != drv_spinor_complete_use_ext(1))
        {
            return ERR_SPINOR_DEVICE_FAILURE;
        }
    }
    ...
    if (0 != (u32_address & ~SPI_NOR_16MB_MASK))
    {
        ...
        if (ERR_SPINOR_SUCCESS != drv_spinor_complete_use_ext(0))
        {
            return ERR_SPINOR_DEVICE_FAILURE;
        }
    }
...
static u8 drv_spinor_complete_read_data_by_riu(u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    do
    {
        if (ERR_SPINOR_SUCCESS != (u8_status = drv_spinor_simple_transmission(&st_spinor_drv_info.u8_readData, 1)))
        {
            break;
        }
        ...
        if (ERR_SPINOR_SUCCESS != (u8_status = drv_spinor_simple_transmission(NULL, u8_dummy)))
        {
            break;
        }
...
static u8 drv_spinor_complete_read_data_by_riu(u32 u32_address, u8 *pu8_data, u32 u32_size)
{
    ...
    do
    {
        ...
        u8_rsize = drv_spinor_write_after_read(au8_cmd, 5, pu8_data, u32_size);
...
static u8 drv_spinor_complete_erase(u8 u8_command, u32 u32_address)
{
    ...
    DRV_QSPI_pull_cs(0);
    u8_status = drv_spinor_simple_transmission(au8_cmd, 4);
    DRV_QSPI_pull_cs(1);
...
static u8 drv_spinor_erase(u8 u8_command, u32 u32_address)
{
    ...
    do
    {
        if (0 != (u32_address & ~SPI_NOR_16MB_MASK))
        {
            ...
            if (ERR_SPINOR_SUCCESS != (u8_status = drv_spinor_complete_use_ext(1)))
            {
                break;
            }
        }
        ...
        if (0 != (u32_address & ~SPI_NOR_16MB_MASK))
        {
            ...
            if (ERR_SPINOR_SUCCESS != (u8_status = drv_spinor_complete_use_ext(0)))
            {
                break;
            }
        }
...
u8 mdrv_spinor_erase(u32 u32_address, u32 u32_size)
{
    ...
    {
        if (0 != (~SPINOR_BLOCK_SIZE_MASK & u32_bytes_left))
        {
