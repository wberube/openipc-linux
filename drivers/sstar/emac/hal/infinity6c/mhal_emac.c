...
static void _MHal_EMAC_Clk(void* hal, int bOn)
{
    if (bOn)
    {
        ...
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x54, 0x00);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x55, 0x00);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x56, 0x00);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x57, 0x00);
    ...
    else
    {
        ...
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x54, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x55, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x56, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x57, 0x01);
...
void MHal_EMAC_Power_On_Clk(void* hal, struct device* dev)
{
    ...
    */
    ...
    {
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x54, 0x04);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x55, 0x00);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x56, 0x04);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_SCGPCTRL, 0x57, 0x00);
    }
...
void MHal_EMAC_Set_Invert_LED(void* hal)
{
    ...
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xf7, u8Reg | BIT0 | BIT3);

    u8Reg = MHal_EMAC_ReadReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xfd);
    // enable reg_led_ctrl
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xfd, u8Reg | BIT1);
}
#endif

#if NETWORK_STORM_PROTECT_DEBUG
void MHal_EMAC_Netsp_Unicast_Setting(void* hal, u32 max, u32 consume, u32 en)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h68;
    MHal_EMAC_WritReg32(hal, REG_EMAA_JULIAN_01A0, max);
    MHal_EMAC_WritReg32(hal, REG_EMAA_JULIAN_01A4, consume);
    EMAC1_h68 = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h68);
    if (en)
    {
    ...
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h68, EMAC1_h68);
}

void MHal_EMAC_Netsp_Multicast_Setting(void* hal, u32 max, u32 consume, u32 en)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h68;
    MHal_EMAC_WritReg32(hal, REG_EMAA_JULIAN_01B0, max);
    MHal_EMAC_WritReg32(hal, REG_EMAA_JULIAN_01B4, consume);
    EMAC1_h68 = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h68);
    if (en)
    {
...
void MHal_EMAC_Netsp_Broadcast_Setting(void* hal, u32 max, u32 consume, u32 en)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h68;
    MHal_EMAC_WritReg32(hal, REG_EMAA_JULIAN_01C0, max);
    MHal_EMAC_WritReg32(hal, REG_EMAA_JULIAN_01C4, consume);
    EMAC1_h68 = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h68);
    if (en)
    {
...
// Flow Control
#if HW_FLOW_CONTROL
void MHal_EMAC_Flow_Control_Mac_Address(void* hal, u8 m0, u8 m1, u8 m2, u8 m3, u8 m4, u8 m5)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          hi   = (u16)m5 << 8 | m4;
    u16          mid  = (u16)m3 << 8 | m2;
    u16          lo   = (u16)m1 << 8 | m0;
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h6D, hi);
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h6E, mid);
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h6F, lo);
}

void MHal_EMAC_Flow_Control_Init(void* hal, u32 isWait, u16 pauseLength)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7D;
    EMAC1_h7D = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D);
    if (isWait)
    {
    ...
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D, EMAC1_h7D);
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h6C, pauseLength);
    // MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h6A, 0x0100); // MAC Opcode
    // MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h6B, 0x0888); // MAC Opcode
    MHal_EMAC_Flow_Control_Mac_Address(hal, 0x01, 0x80, 0xC2, 0x00, 0x00, 0x01); // Multicast Destination Address
}

void MHal_EMAC_TX_Flow_control_sw_triger(void* hal, u32 en)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7D;
    EMAC1_h7D = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D);
...
void MHal_EMAC_TX_Flow_control_hw_triger(void* hal, u32 en)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7D;
    EMAC1_h7D = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D);
...
void MHal_EMAC_Flow_control_Rx_Done_Trig(void* hal)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7D;
    EMAC1_h7D = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D);
...
u16 MHal_EMAC_Read_Rx_Queue_Cnt(void* hal)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7E;
    EMAC1_h7E = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7E);
    EMAC1_h7E &= REG_RX_QUEUE_CNT_FOR_PAUSE;
    return EMAC1_h7E;
}

void MHal_EMAC_Flow_control_Auto_Trig_th(void* hal, u16 th)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7D;
    EMAC1_h7D = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D);
...
void MHal_EMAC_RX_Flow_control_hw_en(void* hal, u32 en)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          EMAC1_h7D;
    EMAC1_h7D = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h7D);
...
static int _MHal_EMAC_TXD_Insert(void* hal, u32 bus, u32 len)
{
    ...
    MHal_EMAC_WritReg8(pHal->emacRIU, REG_BANK_EMAC0, (REG_TXD_XMIT0 + ((write & 0x1) << 1)),
                       1); // Bad, but at least streamming
                           /*
                               ...
                               // MHal_EMAC_WritReg8(REG_BANK_EMAC0, (REG_TXD_XMIT0 + ((write & 0x0) << 1)), 1); // Bad, but at least
                           streamming
                               // MHal_EMAC_WritReg8(REG_BANK_EMAC0, (REG_TXD_XMIT0 + (((write & 0x3)>>1) << 1)), 2); // Bad, but at least
                           streamming
                               // MHal_EMAC_WritReg8(REG_BANK_EMAC0, (REG_TXD_XMIT0 + (((write & 0x3)>>1) << 1)), 1); // Bad, but at least
                           streamming                        #endif
                           */
    ...
    // MHal_EMAC_ReadReg16(REG_BANK_EMAC0, REG_TXD_PTR_L), hal_emac[0].TXD_read);
    return 1;
}
...
#else
#if defined(CONFIG_MS_PADMUX) && defined(EMAC_PADMUX_ENABLE)
...
#if HW_FLOW_CONTROL
    return 1; // I6C Use HW Flow Conrol
#else
    return 0; // I6C Use SW Flow Conrol
#endif
}
...
#if HW_FLOW_CONTROL
void MHal_EMAC_TX_Flow_Ctrl_Enable(void* hal, int enable)
{
    MHal_EMAC_RX_Flow_control_hw_en(hal, enable); // enable/disable hw if stop tx queue when receive a pause frame
}
#endif

#if HW_FLOW_CONTROL
void MHal_EMAC_RX_Flow_Ctrl_Enable(void* hal, int enable)
{
    MHal_EMAC_TX_Flow_control_hw_triger(hal, enable); // enable/disable hw if send to a pause frame when rx
}
#endif
...
u16 MHal_EMAC_SG_IRQ_GET_WPoint(void* hal)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    return (MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h34) & 0x3FF);
}

u16 MHal_EMAC_SG_IRQ_GET_RPoint(void* hal)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    return (MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h35) & 0x3FF);
}
...
void MHal_EMAC_SG_V3(void* hal, int bEnable)
{
    ...
    EMAC1_h74 = MHal_EMAC_ReadReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h74);
    if (bEnable)
    {
        EMAC1_h74 = EMAC1_h74 | SCATTER_DMA_VER;
    }
    else
    {
        EMAC1_h74 = EMAC1_h74 & ~SCATTER_DMA_VER;
    ...
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h74, EMAC1_h74);
}
...
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          table_addr_low, table_addr_high;

    table_addr_low  = addr & 0xFFFF;
    table_addr_high = (addr >> 16) & 0xFFFF;

    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h70, table_addr_low);
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h71, table_addr_high);
}

void MHal_EMAC_SG_V3_PACKET_ADDR(void* hal, u32 addr)
{
    mhal_emac_t* pHal = (mhal_emac_t*)hal;
    u16          packet_addr_low, packet_addr_high;

    packet_addr_low  = addr & 0xFFFF;
    packet_addr_high = (addr >> 16) & 0xFFFF;

    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h72, packet_addr_low);
    MHal_EMAC_WritReg16(pHal->emacRIU, REG_BANK_EMAC1, REG_ETH_EMAC1_h73, packet_addr_high);
}

void MHal_EMAC_SG_V3_FRAG_FILL(u32 addr, u16 len, u16 index, int end)
{
    *(u32*)(0xF9004000 + index * 8 + 4) = (addr >> 20) & 0xFFF;
    *(u32*)(0xF9004000 + index * 8)     = ((addr & 0xFFFFF) << 12) | ((len & 0x7FF) << 1) | end;
}
#endif
