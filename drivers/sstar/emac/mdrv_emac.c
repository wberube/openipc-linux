...
int         MDev_EMAC_SwReset(struct net_device *dev);
...
static void _MDev_EMAC_FlowTX_CB(struct timer_list *t);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
static void _MDev_EMAC_IntRX_CB(struct timer_list *t);
#else
...
static int MDev_EMAC_open(struct net_device *dev)
{
    ...
    if (tx_delay_debug)
    {
        MHal_EMAC_IntEnable(hemac->hal, EMAC_INT_TX_DELAY, tx_delay_enable);
        MHal_EMAC_IntEnable(hemac->hal, EMAC_INT_TX_DELAY_TIMEOUT, tx_delay_timeout_enable);
        MHal_EMAC_WritReg32(hemac->hal, REG_TXDELAY_MAX_CNT, tx_delay_num);
        MHal_EMAC_WritReg32(hemac->hal, REG_TXDELAY_TO_MAX_CNT, tx_delay_timeout);
    }
#endif
...
static void _MDrv_EMAC_PausePkt_Send(struct net_device *dev)
{
    ...
    Chip_Flush_Cache_Range((void *)hemac->pu8PausePkt, hemac->u8PausePktSize);
    spin_lock_irqsave(&hemac->mutexTXQ, flags);
    hemac->isPausePkt = 1;
...
static int MDev_EMAC_tx(struct sk_buff *skb, struct net_device *dev)
{
...
#if EMAC_SG
    {
        ...
        if (nr_frags)
        {
            ...
            */
            if (EMAC_SG_BUF_CACHE)
                Chip_Flush_Cache_Range((void *)start, skb->len);
        ...
        else
        {
            ...
            spin_unlock_irqrestore(&hemac->mutexTXQ, flags);
            Chip_Flush_Cache_Range((void *)skb->data, skb->len);
        }
...
#if SCATTER_DMA && !SCATTER_DMA_BYPASS
    {
        ...
        w_point = MHal_EMAC_SG_IRQ_GET_RPoint(hemac->hal) & 0x3FF;
        r_point = MHal_EMAC_SG_IRQ_GET_RPoint(hemac->hal) & 0x3FF;
        if ((w_point + 1024 - r_point) % 1024 + nr_frags > 512)
        {
            printk("[%s][%d] scatterdma table full\n", __FUNCTION__, __LINE__);
        }
        ...
        if (nr_frags)
        {
            ...
            Chip_Flush_Cache_Range((void *)start, skb_headlen(skb));
            skb_addr = VIRT2BUS(start);
            ...
            for (i = 0; i < nr_frags; i++)
            {
                ...
                Chip_Flush_Cache_Range((void *)frag_addr, skb_frag_size(frag));
                skb_addr = VIRT2BUS(frag_addr);
                MHal_EMAC_SG_WRITE(hemac->hal, skb_addr, skb_frag_size(frag), nr_frags, i + 1);
#if SCATTER_DMA_V3
                MHal_EMAC_SG_V3_FRAG_FILL(skb_addr, skb_frag_size(frag), (SC_idx + 1 + i) % SC_IDX_MAX,
                                          ((i + 1) == nr_frags) ? 1 : 0);
#endif
            }
...
static int MDev_EMAC_rx(struct net_device *dev, int budget)
{
    ...
    do
    {
        ...
        if (IS_EXT_PHY(hemac))
        {
            dma_unmap_single(hemac->dev, VIRT2PA(p_recv), pktlen, DMA_FROM_DEVICE);
        ...
        */
        ...
        if (_ethtool_rx_pause)
        {
            MHal_EMAC_Flow_control_Rx_Done_Trig(hemac->hal);
        }
#endif
        ...
        if (IS_EXT_PHY(hemac))
        {
            ...
            desc->addr = CLR_BITS(desc->addr, EMAC_DESC_DONE);
            dma_map_single(hemac->dev, p_recv, EMAC_PACKET_SIZE_MAX, DMA_FROM_DEVICE);
        }
        else
#endif
        {
...
static int MDev_EMAC_napi_poll(struct napi_struct *napi, int budget)
{
    ...
    if (hemac->rx_stats_enable)
    {
       ...
       // (int)elapse, (int)hemac->rx_stats_packet, (int)packets); MHal_EMAC_RX_ParamSet(hemac->hal, packets/200 + 2,
       // 0xFFFFFFFF);
...
void MDev_EMAC_HW_init(struct net_device *dev)
{
    ...
    // Set MAC address ------------------------------------------------------
    MHal_EMAC_Write_SA1_MAC_Address(hemac->hal, hemac->sa[0][0], hemac->sa[0][1], hemac->sa[0][2], hemac->sa[0][3],
                                    hemac->sa[0][4], hemac->sa[0][5]);
    MHal_EMAC_Write_SA2_MAC_Address(hemac->hal, hemac->sa[1][0], hemac->sa[1][1], hemac->sa[1][2], hemac->sa[1][3],
                                    hemac->sa[1][4], hemac->sa[1][5]);
    MHal_EMAC_Write_SA3_MAC_Address(hemac->hal, hemac->sa[2][0], hemac->sa[2][1], hemac->sa[2][2], hemac->sa[2][3],
                                    hemac->sa[2][4], hemac->sa[2][5]);
    MHal_EMAC_Write_SA4_MAC_Address(hemac->hal, hemac->sa[3][0], hemac->sa[3][1], hemac->sa[3][2], hemac->sa[3][3],
...
static void sstar_emac_get_ethtool_stats(struct net_device *dev, struct ethtool_stats *stats, u64 *data)
{
    ...
    emac_stats = MDev_EMAC_stats(dev);
...
static int sstar_emac_set_pause_param(struct net_device *ndev, struct ethtool_pauseparam *pauseparam)
{
    ...
    if (_ethtool_tx_pause)
    {
        MHal_EMAC_TX_Flow_Ctrl_Enable(hemac->hal, TRUE);
    }
    else
    {
        MHal_EMAC_TX_Flow_Ctrl_Enable(hemac->hal, FALSE);
    }
    if (_ethtool_rx_pause)
    {
        MHal_EMAC_RX_Flow_Ctrl_Enable(hemac->hal, TRUE);
    }
    else
    {
        MHal_EMAC_RX_Flow_Ctrl_Enable(hemac->hal, FALSE);
    }
#endif
...
static int MDev_EMAC_set_features(struct net_device *netdev, netdev_features_t features)
{
    /*
    printk("[%s][%d] ##################################\n", __FUNCTION__, __LINE__);
...
static netdev_features_t MDev_EMAC_fix_features(struct net_device *netdev, netdev_features_t features)
{
    /*
    printk("[%s][%d] ##################################\n", __FUNCTION__, __LINE__);
...
#define CHANGE_LINE_LENG 0x20
    ...
    */
    ...
    /*
        ...
        str += scnprintf(str, end - str, "(size0, size1) = (%d, %d)\n", hemac->skb_queue_tx.size[0],
       hemac->skb_queue_tx.size[1]); str += scnprintf(str, end - str, "read =           %d\n",
       hemac->skb_queue_tx.read); str += scnprintf(str, end - str, "rw =             %d\n", hemac->skb_queue_tx.rw); str
       += scnprintf(str, end - str, "write =          %d\n", hemac->skb_queue_tx.write);
    */
    {
        struct timespec64 ct;
        int               duration;
...

    {
        ...
        if (2 == sscanf(buf, "%s %d", cmd, &param))
        {
            if (!strncmp(cmd, "swing_100", strlen("swing_100")))
            {
                ...
                // enable
                MHal_EMAC_read_phy(hemac->hal, 0, 0x02e, &val); // Bank: phy0_2e
                val = val | 0x1;
                MHal_EMAC_write_phy(hemac->hal, 0, 0x02e, val);

                // swing
                MHal_EMAC_read_phy(hemac->hal, 0, 0x142, &val); // Bank: phy2_42
                tmp = ((val >> 8) & 0x1f);
...
#if PHY_REGISTER_DEBUG || NETWORK_STORM_PROTECT_DEBUG || HW_FLOW_CONTROL_DEBUG
...
static ssize_t phyStatusWR_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    else if (strcmp(argv[0], "phyE_r") == 0)
    {
        ...
        MHal_EMAC_read_phy(hemac->hal, phyId, phyAddress, &phyValue);
    ...
    else if (strcmp(argv[0], "phyE_w") == 0)
    {
        ...
        MHal_EMAC_write_phy(hemac->hal, phyId, phyAddress, phyValue);
...
static ssize_t netStormProtect_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (strcmp(argv[0], "unicast") == 0)
    {
        ...
        MHal_EMAC_Netsp_Unicast_Setting(hemac->hal, net_storm_proctect_var.uni_cast_max,
    ...
    else if (strcmp(argv[0], "multicast") == 0)
    {
        ...
        MHal_EMAC_Netsp_Multicast_Setting(hemac->hal, net_storm_proctect_var.multi_cast_max,
    ...
    else if (strcmp(argv[0], "broadcast") == 0)
    {
        ...
        MHal_EMAC_Netsp_Broadcast_Setting(hemac->hal, net_storm_proctect_var.broad_cast_max,
    ...
    else if (strcmp(argv[0], "preset") == 0)
    {
        if (strcmp(argv[1], "unicast") == 0)
        {
            ...
            MHal_EMAC_Netsp_Unicast_Setting(hemac->hal, TX_MAX_DEFAULT, _levet2consume(level), 1);
            printk("unicast set level[%d]\n", level);
        }
        else if (strcmp(argv[1], "multicast") == 0)
        {
            level = simple_strtoul(argv[2], NULL, 10);
            MHal_EMAC_Netsp_Multicast_Setting(hemac->hal, TX_MAX_DEFAULT, _levet2consume(level), 1);
            printk("multicast set level[%d]\n", level);
        }
        else if (strcmp(argv[1], "broadcast") == 0)
        {
            level = simple_strtoul(argv[2], NULL, 10);
            MHal_EMAC_Netsp_Broadcast_Setting(hemac->hal, TX_MAX_DEFAULT, _levet2consume(level), 1);
...
static ssize_t flowControl_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (strcmp(argv[0], "rx") == 0)
    {
        ...
        MHal_EMAC_TX_Flow_Ctrl_Enable(hemac->hal, rx_flow_en);
    }
    else if (strcmp(argv[0], "tx") == 0)
    {
        if (strcmp(argv[1], "hw") == 0)
        {
            ...
            MHal_EMAC_RX_Flow_Ctrl_Enable(hemac->hal, tx_flow_en);
        }
        else if (strcmp(argv[1], "sw") == 0)
        {
            ...
            {
                MHal_EMAC_TX_Flow_control_sw_triger(hemac->hal, 1);
                CamOsUsDelay(1);
                MHal_EMAC_TX_Flow_control_sw_triger(hemac->hal, 0);
            }
        }
    }
    else if (strcmp(argv[0], "init") == 0)
    {
        ...
        MHal_EMAC_Flow_Control_Init(hemac->hal, isWait, pauseLength);
    }
...
#if HW_FLOW_CONTROL
        MHal_EMAC_Flow_Control_Mac_Address(hemac->hal, m0, m1, m2, m3, m4, m5);
#endif
    }
    else if (strcmp(argv[0], "th") == 0)
    {
        ...
        MHal_EMAC_Flow_control_Auto_Trig_th(hemac->hal, th);
    }
    return count;
}
...
#if TX_DELAY_INT_DEBUG
1 static ssize_t tx_delay_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *str = buf;
    char *end = buf + PAGE_SIZE;
...
static ssize_t tx_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    tx_delay_debug          = simple_strtoul(argv[0], NULL, 16);
    tx_delay_enable         = simple_strtoul(argv[1], NULL, 16);
    tx_delay_timeout_enable = simple_strtoul(argv[2], NULL, 16);
    tx_delay_num            = simple_strtoul(argv[3], NULL, 16);
    tx_delay_timeout        = simple_strtoul(argv[4], NULL, 16);
...
static int MDev_EMAC_setup(struct net_device *dev)
{
    ...
    spin_lock_irqsave(&hemac->mutexTXQ, flags);
    skb_queue_create(&(hemac->skb_queue_tx), MHal_EMAC_TXQ_Size(hemac->hal),
                     MHal_EMAC_TXQ_Size(hemac->hal) + hemac->txq_num_sw);
    spin_unlock_irqrestore(&hemac->mutexTXQ, flags);

#if EMAC_FLOW_CONTROL_TX
    spin_lock_init(&hemac->mutexFlowTX);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
    timer_setup(&hemac->timerFlowTX, _MDev_EMAC_FlowTX_CB, 0);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
    timer_setup(&hemac->timerIntRX, _MDev_EMAC_IntRX_CB, 0);
...
#endif
#if NETWORK_STORM_PROTECT_DEBUG
    device_create_file(hemac->mstar_class_emac_device, &dev_attr_netStormProtect);
#endif
#if HW_FLOW_CONTROL_DEBUG
    device_create_file(hemac->mstar_class_emac_device, &dev_attr_flowControl);
#endif
#if TX_DELAY_INT_DEBUG
    device_create_file(hemac->mstar_class_emac_device, &dev_attr_tx_delay);
#endif
#if 0 // ajtest
...
static int MDev_EMAC_init(struct platform_device *pdev)
{
    ...
    */
    ...
    if (0 == _phyReset)
    {
        ...
        {
            int gpio_no;
            if (PAD_UNKNOWN != (gpio_no = mdrv_padmux_getpad(MDRV_PUSE_ETH0_PHY_RESET)))
            {
                MDrv_GPIO_Set_High(gpio_no);
                printk("[%s] EMAC0 PHY RESET \n", __FUNCTION__);
            }
            if (PAD_UNKNOWN != (gpio_no = mdrv_padmux_getpad(MDRV_PUSE_ETH1_PHY_RESET)))
            {
                MDrv_GPIO_Set_High(gpio_no);
    ...
    */
    ...
    if (_ethtool_rx_pause)
    {
        ...
        MHal_EMAC_Flow_Control_Init(hemac->hal, 1, 0xffff);               // send pause frame onece 1ms
        MHal_EMAC_RX_Flow_Ctrl_Enable(hemac->hal, 0);                     // disable hw triger
        MHal_EMAC_RX_Flow_Ctrl_Enable(hemac->hal, 1);                     // enable hw triger
        MHal_EMAC_Flow_control_Auto_Trig_th(hemac->hal, RX_DESC_NUM - 1); // set auto trig threshold
    }
    if (_ethtool_tx_pause)
    {
        MHal_EMAC_TX_Flow_Ctrl_Enable(hemac->hal,
...
#endif
{
    ...
    spin_lock_irqsave(&hemac->mutexIntRX, flags);
    MHal_EMAC_RX_ParamSet(hemac->hal, 0x01, 0x10);
...
static struct platform_driver Mstar_emac_driver = {.probe   = mstar_emac_drv_probe,
