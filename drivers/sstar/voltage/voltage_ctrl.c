...
struct voltage_ctrl
{
    ...
    struct mutex            core_voltage_mutex;
...
static void sync_core_voltage(struct voltage_ctrl *ctrl)
{
    ...
    mutex_lock(&ctrl->core_voltage_mutex);
    ...
    mutex_unlock(&ctrl->core_voltage_mutex);
}
...
static int core_voltage_get_gpio(struct voltage_ctrl *ctrl)
{
    ...
    if (0 != of_property_read_u32(np, "vid_width", &ctrl->gpio_vid_width) || ctrl->gpio_vid_width < 1)
        goto core_voltage_get_gpio_err;

    ctrl->gpio_vid_pins = vzalloc(sizeof(unsigned int) * ctrl->gpio_vid_width);
    if (!ctrl->gpio_vid_pins)
        goto core_voltage_get_gpio_err;

    ctrl->gpio_vid_voltages = vzalloc(sizeof(unsigned int) * (1 << ctrl->gpio_vid_width));
    if (!ctrl->gpio_vid_voltages)
        goto core_voltage_get_gpio_err;

    if (ctrl->gpio_vid_width
        != of_property_read_variable_u32_array(np, "vid_gpios", ctrl->gpio_vid_pins, 0, ctrl->gpio_vid_width))
        goto core_voltage_get_gpio_err;
    if ((1 << ctrl->gpio_vid_width)
        != of_property_read_variable_u32_array(np, "vid_voltages", ctrl->gpio_vid_voltages, 0,
                                               (1 << ctrl->gpio_vid_width)))
        goto core_voltage_get_gpio_err;
    for (i = 0; i < ctrl->gpio_vid_width; i++)
    {
        sprintf(name, "vid%d", i);
        ret = gpio_request(ctrl->gpio_vid_pins[i], (const char *)name);
        if (ret)
            goto core_voltage_get_gpio_err;
        gpio_export(ctrl->gpio_vid_pins[i], 0);
    }
...
static int sstar_voltage_ctrl_probe(struct platform_device *pdev)
{
    ...
    mutex_init(&ctrl->core_voltage_mutex);
