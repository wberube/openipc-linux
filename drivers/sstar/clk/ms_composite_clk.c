...
static void __init ms_clk_composite_init(struct device_node *node)
{
    ...
    {
        clk = clk_register_composite(NULL, clk_name, parent_names, num_parents, &mux->hw, &ms_clk_mux_ops, &mux->hw,
                                     &ms_clk_mux_ops, &gate->hw, &clk_gate_ops, flag);
    }
    else if (mux->shift != 0xFF)
    {
        clk = clk_register_composite(NULL, clk_name, parent_names, num_parents, &mux->hw, &ms_clk_mux_ops, &mux->hw,
                                     &ms_clk_mux_ops, NULL, NULL, flag);
        kfree(gate);
        gate = NULL;
    }
