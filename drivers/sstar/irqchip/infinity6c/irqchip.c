...
static void ms_main_irq_ack(struct irq_data *d)
{
    ...
    {
        SETREG16_BIT_OP((BASE_REG_INTRCTL_PA + REG_ID_4C + (ms_fiq / 16) * 4), (1 << (ms_fiq % 16)));
...
static void ms_main_irq_mask(struct irq_data *d)
{
    ...
    ms_fiq = d->hwirq - GIC_SPI_ARM_INTERNAL_NR - GIC_SPI_MS_IRQ_NR;
    pr_debug("[%s]  hwirq %lu %s=%d\n", __func__, d->hwirq, (ms_fiq >= 0) ? "ms_fiq" : "ms_irq",
             (ms_fiq >= 0) ? ms_fiq : ms_irq);
...
static int ms_main_intc_domain_translate(struct irq_domain *domain, struct irq_fwspec *fwspec, unsigned long *hwirq,
                                         unsigned int *type)
{
...
static int ms_main_intc_domain_alloc(struct irq_domain *domain, unsigned int virq, unsigned int nr_irqs, void *data)
{
...
static int ms_irqchip_suspend(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        if (i >= 0)
        {
            ms_main_irq_priv.irq_mask[i]     = INREG16(BASE_REG_INTRCTL_PA + REG_ID_54 + (i << 2));
            ms_main_irq_priv.irq_polarity[i] = INREG16(BASE_REG_INTRCTL_PA + REG_ID_58 + (i << 2));
        }
        //        else
        //        {
        //            ms_main_irq_priv.irq_mask[i] = INREG16(BASE_REG_INTRCTL2_PA + REG_ID_54 + ((i -
        //            INT_IRQ_BANK2_BASE) << 2)); ms_main_irq_priv.irq_polarity[i] =
        //                INREG16(BASE_REG_INTRCTL2_PA + REG_ID_58 + ((i - INT_IRQ_BANK2_BASE) << 2));
        //        }
    }
    num = (GIC_SPI_MS_FIQ_NR) >> 4;
    for (i = 0; i < num; i++)
    {
        ms_main_irq_priv.fiq_mask[i]     = INREG16(BASE_REG_INTRCTL_PA + REG_ID_44 + (i << 2));
...
static void ms_irqchip_resume(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        if (i >= 0)
        {
            OUTREG16(BASE_REG_INTRCTL_PA + REG_ID_54 + (i << 2), ms_main_irq_priv.irq_mask[i]);
            OUTREG16(BASE_REG_INTRCTL_PA + REG_ID_58 + (i << 2), ms_main_irq_priv.irq_polarity[i]);
        }
        //        else
        //        {
        //            OUTREG16(BASE_REG_INTRCTL2_PA + REG_ID_54 + ((i - INT_IRQ_BANK2_BASE) << 2),
        //            ms_main_irq_priv.irq_mask[i]); OUTREG16(BASE_REG_INTRCTL2_PA + REG_ID_58 + ((i -
        //            INT_IRQ_BANK2_BASE) << 2),
        //                     ms_main_irq_priv.irq_polarity[i]);
        //        }
    }
    ...
    for (i = 0; i < num; i++)
    {
        OUTREG16(BASE_REG_INTRCTL_PA + REG_ID_44 + (i << 2), ms_main_irq_priv.fiq_mask[i]);
