...
static int parent_gpi_virt_irq = 0;
static int parent_gpi_hw_irq   = 0;
...
static void ss_gpi_irq_ack(struct irq_data *d)
{
    ...
    if (gpi_irq >= 0 && gpi_irq < (GPI_FIQ_NUM + GPI_GPIC_NUM))
    {
        ...
        {
            SETREG16((BASE_REG_GPI_INT2_PA + REG_ID_7A + ((gpi_irq - GPI_GPIC_START) / 16) * 4),
 ...
 */
static int ss_gpi_irq_retrigger(struct irq_data *d)
{
...
struct irq_chip ss_gpi_intc_irqchip = {
    ...
    .irq_retrigger = ss_gpi_irq_retrigger,
};
...
static int ss_gpi_intc_domain_translate(struct irq_domain *domain, struct irq_fwspec *fwspec, unsigned long *hwirq,
                                        unsigned int *type)
{
...
static int ss_gpi_intc_domain_alloc(struct irq_domain *domain, unsigned int virq, unsigned int nr_irqs, void *data)
{
    ...
    parent_fwspec.param[1]    = parent_gpi_hw_irq;
...
static int ss_gpi_intc_suspend(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        gpi_irq_priv.gpic_polarity[i] = INREG16(BASE_REG_GPI_INT2_PA + REG_ID_7B + (i << 2));
    }
...
static void ss_gpi_intc_resume(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        OUTREG16(BASE_REG_GPI_INT2_PA + REG_ID_7B + (i << 2), gpi_irq_priv.gpic_polarity[i]);
    }
...
static int __init ss_init_gpi_intc(struct device_node *np, struct device_node *interrupt_parent)
{
    ...
    parent_gpi_hw_irq   = oirq.args[1];
    parent_gpi_virt_irq = irq_create_of_mapping(&oirq);

    if (!parent_gpi_virt_irq)
    {
        pr_err("Get irq err from DTS\n");
        return -EPROBE_DEFER;
    }

    irq_set_chained_handler_and_data(parent_gpi_virt_irq, ss_handle_cascade_gpi, ss_gpi_irq_domain);
