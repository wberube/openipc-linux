...
#define UART_DEBUG            0
#define MS_UART_8250_BUG_THRE 1
...
#define REG_DLL_THR_RBR(p) GET_REG_ADDR((unsigned long)p->membase, (0x00))
#define REG_DLH_IER(p)     GET_REG_ADDR((unsigned long)p->membase, (0x02))
#define REG_IIR_FCR(p)     GET_REG_ADDR((unsigned long)p->membase, (0x04))
#define REG_LCR(p)         GET_REG_ADDR((unsigned long)p->membase, (0x06))
#define REG_MCR(p)         GET_REG_ADDR((unsigned long)p->membase, (0x08))
#define REG_LSR(p)         GET_REG_ADDR((unsigned long)p->membase, (0x0A))
#define REG_MSR(p)         GET_REG_ADDR((unsigned long)p->membase, (0x0C))
#define REG_USR(p)         GET_REG_ADDR((unsigned long)p->membase, (0x0E))
#define REG_TFL(p)         GET_REG_ADDR((unsigned long)p->membase, (0x10))
#define REG_RFL(p)         GET_REG_ADDR((unsigned long)p->membase, (0x12))
#define REG_RST(p)         GET_REG_ADDR((unsigned long)p->membase, (0x14))
...
#define URDMA_LOU16(u64Val) ((U16)(u64Val)&0xFFFF)
#define URDMA_HIU16(u64Val) ((U16)((u64Val) >> 16) & 0xFFFF)
#define URDMA_MSU16(u64Val) ((U16)((u64Val) >> 32) & 0x000F)
...
typedef struct reg_urdma
{
    ...
    union
    {
        ...
        struct
        {
            volatile U16 reserve0c : 16; /* BIT0 ~ BIT15 */
        };
    };
    U16 space0e;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 reserve0d : 16; /* BIT0 ~ BIT15 */
        };
    };
    U16 space0f;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 clk_uart_en : 1;   /* BIT0 */
            volatile U16 clk_miu_en : 1;    /* BIT1 */
            volatile U16 reserve0e : 12;    /* BIT2 ~ BIT13 */
            volatile U16 urdma2miu_sel : 2; /* BIT14 ~ BIT15 */
        };
    };
    U16 space10;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 reserve0f : 16; /* BIT0 ~ BIT15 */
        };
    };
    U16 space11;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 reserve10 : 16; /* BIT0 ~ BIT15 */
        };
    };
    U16 space12;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 reserve11 : 16; /* BIT0 ~ BIT15 */
        };
    };
    U16 space13;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 tx_buf_base_msb : 4; /* BIT0 ~ BIT3 */
            volatile U16 reserve12 : 12;      /* BIT4 ~ BIT15 */
        };
    };
    U16 space14;
    ...
    union
    {
        ...
        struct
        {
            volatile U16 rx_buf_base_msb : 4; /* BIT0 ~ BIT3 */
            volatile U16 reserve13 : 12;      /* BIT4 ~ BIT15 */
        };
    };
    U16 space15;
} reg_urdma;
...
struct ms_urdma
{
    ...
    u32             rx_urdma_size;
    u32             tx_urdma_size;
    phys_addr_t     tx_urdma_phys;
    phys_addr_t     rx_urdma_phys;
    u16             sw_rx_rptr;
    MSYS_DMEM_INFO *rx_msys_str;
    MSYS_DMEM_INFO *tx_msys_str;
};
...
static volatile int urdma_tx_first = 0;
...
static int ms_uart_rs485_config(struct uart_port *port, struct serial_rs485 *rs485)
{
...
static int ms_uart_rs485_handle(void *pdata)
{
    struct ms_uart_port *mp = (struct ms_uart_port *)pdata;
...
void inline ms_uart_clear_fifos(struct uart_port *p)
{
    ...
    if (timeout == 2000)
        UART_ERR("Clean FIFO failed, uart waveform may wrong!\n");
}

void ms_force_rx_disable(u8 line, BOOL status)
{
    switch (line) // set 0 for disable, 1 for enable
    {
        case 0: // uart0
            OUTREGMSK16(REG_FORCE_RX_DISABLE, (~status) << UART0_RX_DIS, 1 << UART0_RX_DIS);
            break;
        case 1: // uart1
            OUTREGMSK16(REG_FORCE_RX_DISABLE, (~status) << UART1_RX_DIS, 1 << UART1_RX_DIS);
            break;
        case 2: // fuart
            OUTREGMSK16(REG_FORCE_RX_DISABLE, (~status) << FUART_RX_DIS, 1 << FUART_RX_DIS);
            break;
        default:
            UART_ERR("[%s] UART %d RX disable unknow\n", __func__, line);
            break;
    }
}
    ...
    else
    {
        ...
        if (pm_uart)
        {
            ...
            if ((abs(real_baud - request_baud) * 100 / request_baud) < tolerance)
            {
                ...
                // request_baud, real_baud, divisor);
            }
            else
            {
                divisor = 0;
            ...
            for (i = 0; i < num_parents; i++)
            {
                if (mp->stCfg.u32Parent[i])
                {
                    ...
                    if ((abs(real_baud - request_baud) * 100 / request_baud) < tolerance)
                    {
                        ...
                        p->uartclk = rate;
                        UART_DBG("[uart%d] uartclk=%d, request_baud=%d, real_baud=%d, divisor=0x%X\n", p->line,
                                 p->uartclk, request_baud, real_baud, divisor);
                        break;
                    }
                }
            }
...
static s32 _ms_uart_console_prepare(int idx)
{
    ...
    lpae_base                 = (unsigned long)res.start;
...
static irqreturn_t ms_uart_interrupt(s32 irq, void *dev_id)
{
    ...
    else
    {
        ...
        else if (iir_fcr == UART_IIR_NO_INT) /* No pending interrupts */
        {
            ...
            //            if (count == retry)
            //                UART_ERR("UART%d Interrupt: No IRQ rasied by UART, but come in to UART ISR. IIR:0x%02X
...
{
    struct ms_uart_port *up = from_timer(up, t, timer);
    u16                  iir, ier, lsr;
    unsigned long        flags;
    ...
    if ((iir & UART_IIR_NO_INT) && (ier & UART_IER_THRI) && (lsr & UART_LSR_TEMT))
    {
    ...
    mod_timer(&up->timer, jiffies + uart_poll_timeout(&up->port) + HZ / 5);
}
...
static s32 ms_uart_startup(struct uart_port *p)
{
    ...
    ms_force_rx_disable(p->line, DISABLE);
    ...
    ms_force_rx_disable(p->line, ENABLE);
#if MS_UART_8250_BUG_THRE
    mp->bugs = (mp->use_dma) ? 0 : UART_BUG_THRE;
#endif
    /*
    ...
    if (mp->bugs & UART_BUG_THRE)
    {
        ...
        timer_setup(&mp->timer, serial8250_backup_timeout, 0);
        mod_timer(&mp->timer, jiffies + uart_poll_timeout(&mp->port) + HZ / 5);
    }
    return rc;
}
...
static s32 ms_uart_dmabuf_alloc(struct ms_urdma *urdma, struct uart_port *port)
{
    s32 s32Ret = 0;

    /* alloc URDMA TX dma mem*/
    urdma->tx_msys_str->length = PAGE_ALIGN(UR2DMA_TX_BUF_LENGTH);
    scnprintf(urdma->tx_msys_str->name, sizeof(urdma->tx_msys_str->name), "Sstar URDMA %d TX", port->line);
    s32Ret = msys_request_dmem(urdma->tx_msys_str);
    if (s32Ret)
    {
        UART_ERR("URDMA-%d alloc tx mem failed\n", port->line);
        goto out_tx;
    }
    urdma->tx_buf        = (u8 *)urdma->tx_msys_str->kvirt;
    urdma->tx_urdma_phys = (phys_addr_t)urdma->tx_msys_str->phys;
    urdma->tx_urdma_base = (dma_addr_t)Chip_Phys_to_MIU((ss_phys_addr_t)urdma->tx_urdma_phys);
    urdma->tx_urdma_size = PAGE_ALIGN(UR2DMA_TX_BUF_LENGTH);
    UART_DBG("URDMA-%d alooc TX mem success! phy = 0x%x, miu adr = 0x%x\n", port->line, urdma->tx_urdma_phys,
             urdma->tx_urdma_base);

    /* alloc URDMA RX dma mem*/
    urdma->rx_msys_str->length = PAGE_ALIGN(UR2DMA_RX_BUF_LENGTH);
    scnprintf(urdma->rx_msys_str->name, sizeof(urdma->rx_msys_str->name), "Sstar URDMA %d RX", port->line);
    s32Ret = msys_request_dmem(urdma->rx_msys_str);
    if (s32Ret)
    {
        UART_ERR("urdma-%d alloc rx mem failed\n", port->line);
        goto out_rx;
    }
    urdma->rx_buf        = (u8 *)urdma->rx_msys_str->kvirt;
    urdma->rx_urdma_phys = (phys_addr_t)urdma->rx_msys_str->phys;
    urdma->rx_urdma_base = (dma_addr_t)Chip_Phys_to_MIU((ss_phys_addr_t)urdma->rx_urdma_phys);
    urdma->rx_urdma_size = PAGE_ALIGN(UR2DMA_RX_BUF_LENGTH);
    UART_DBG("URDMA-%d alooc RX mem success! phy = 0x%x, miu adr = 0x%x\n", port->line, urdma->rx_urdma_phys,
             urdma->rx_urdma_base);

    return s32Ret;
out_rx:
    msys_release_dmem(urdma->tx_msys_str);
out_tx:
    memset((void *)urdma, 0, sizeof(struct ms_urdma));
    return s32Ret;
}
...
static s32 ms_uart_remove(struct platform_device *pdev)
{
    ...
    if (mp->use_dma)
    {
        msys_release_dmem(mp->urdma->rx_msys_str);
        msys_release_dmem(mp->urdma->tx_msys_str);
...
static s32 ms_uart_probe(struct platform_device *pdev)
{
    ...
    if (mp->use_dma)
    {
        ...
        /*
        mp->urdma->rx_buf =
            dma_alloc_coherent(&pdev->dev, PAGE_ALIGN(UR2DMA_RX_BUF_LENGTH), &mp->urdma->rx_urdma_base, GFP_KERNEL);
        mp->urdma->tx_buf =
            dma_alloc_coherent(&pdev->dev, PAGE_ALIGN(UR2DMA_TX_BUF_LENGTH), &mp->urdma->tx_urdma_base, GFP_KERNEL);
        ...
        mp->urdma->rx_urdma_base = Chip_Phys_to_MIU(mp->urdma->rx_urdma_base);
        mp->urdma->tx_urdma_base = Chip_Phys_to_MIU(mp->urdma->tx_urdma_base);
        mp->urdma->rx_urdma_size = PAGE_ALIGN(UR2DMA_RX_BUF_LENGTH);
        mp->urdma->tx_urdma_size = PAGE_ALIGN(UR2DMA_TX_BUF_LENGTH);
        UART_DBG("[%s] URDMA mode enable, reg_base=0x%08X, irq=%d\n", __func__, (unsigned int)(mp->urdma->reg_base),
                 mp->urdma->urdma_irq);
        // UART_ERR("URDMA rx_buf=0x%08X(phy:0x%08X) tx_buf=0x%08X(phy:0x%08X) size=0x%X\n", (unsigned
        // int)mp->urdma->rx_buf, (unsigned int)mp->urdma->rx_urdma_base, (unsigned int)mp->urdma->tx_buf, (unsigned
        // int)mp->urdma->tx_urdma_base, mp->urdma->rx_urdma_size);
        */
        mp->urdma->rx_msys_str = NULL;
        mp->urdma->tx_msys_str = NULL;
        mp->urdma->rx_msys_str = (MSYS_DMEM_INFO *)devm_kzalloc(&pdev->dev, sizeof(MSYS_DMEM_INFO), GFP_KERNEL);
        if (!mp->urdma->rx_msys_str)
        {
            goto dma_err;
        }
        mp->urdma->tx_msys_str = (MSYS_DMEM_INFO *)devm_kzalloc(&pdev->dev, sizeof(MSYS_DMEM_INFO), GFP_KERNEL);
        if (!mp->urdma->tx_msys_str)
        {
            devm_kfree(&pdev->dev, mp->urdma->rx_msys_str);
            goto dma_err;
        }
        if (ms_uart_dmabuf_alloc(mp->urdma, &mp->port))
        {
            mp->use_dma = 0;
        }
    dma_err:
    ...
    mp->port.has_sysrq = IS_ENABLED(CONFIG_MAGIC_SYSRQ_SERIAL);
#ifdef SOFT_RS485
    mp->port.rs485_config = ms_uart_rs485_config;
    mp->rs485_thr         = kthread_create(ms_uart_rs485_handle, mp, "rs485_thread");
#endif
...
static const struct of_device_id ms_uart_of_match_table[] = {{.compatible = "sstar,uart"}, {}};
...
void URDMA_TxInit(struct uart_port *p)
{
    ...
    mp->urdma->reg_base->tx_buf_base_h   = URDMA_HIU16((U64)mp->urdma->tx_urdma_base);
    mp->urdma->reg_base->tx_buf_base_l   = URDMA_LOU16((U64)mp->urdma->tx_urdma_base);
    mp->urdma->reg_base->tx_buf_base_msb = URDMA_MSU16((U64)mp->urdma->tx_urdma_base);
...
void URDMA_RxInit(struct uart_port *p)
{
    ...
    mp->urdma->reg_base->rx_buf_base_h   = URDMA_HIU16((U64)mp->urdma->rx_urdma_base);
    mp->urdma->reg_base->rx_buf_base_l   = URDMA_LOU16((U64)mp->urdma->rx_urdma_base);
    mp->urdma->reg_base->rx_buf_base_msb = URDMA_MSU16((U64)mp->urdma->rx_urdma_base);
...
U32 URDMA_GetWritableSize(struct uart_port *p)
{
    ...
    return (U32)((mp->urdma->reg_base->tx_buf_rptr - mp->urdma->reg_base->tx_buf_wptr - 1) & (mp->urdma->rx_urdma_size -
1));
}
*/
static int urdma_tx_thread(void *arg)
{
    ...
    while (!kthread_should_stop())
    {
        wait_event_interruptible(urdma_wait, mp->urdma_conditions);
        mp->urdma_conditions = 0;
        xmit                 = &p->state->xmit;
        if (uart_circ_empty(xmit) || uart_tx_stopped(p))
        {
...
static int _urdma_tx(struct ms_uart_port *mp, unsigned char *buf, int buf_size)
{
    ...
    if (isConsole && silent_state)
    {
        spin_unlock_irqrestore(&mutex_console_2_dma, flags1);
        return buf_size;
    }

    if (mp->urdma->reg_base->tx_buf_wptr == 0 && mp->urdma->reg_base->tx_buf_rptr == 0 && urdma_tx_first == 1)
    {
        sw_tx_wptr = (sw_tx_wptr)&UR2DMA_TX_BUF_MASK;
        if (buf_size == 1)
            UART_ERR("The number of fuart send must more than 1 Byte!\n");
    }
    else
    {
    ...
    if (complete_size)
    {
        ...
        {
            // in first send 1byte after reset,you need set tx_buf_wptr as 0,or it will send 2 bytes out
            // then after,add size as you want sending
            if ((0 == urdma_tx_first) && (1 == complete_size))
            {
                mp->urdma->reg_base->tx_buf_wptr =
                    (mp->urdma->reg_base->tx_buf_wptr + complete_size) & UR2DMA_TX_BUF_MASK;
            }
            else
            {
                mp->urdma->reg_base->tx_buf_wptr =
                    ((mp->urdma->reg_base->tx_buf_wptr + complete_size - 1) & UR2DMA_TX_BUF_MASK);
                urdma_tx_first = 0;
            }
        }
        else
...
void URDMA_StartTx(struct uart_port *p)
{
    ...
    U16                  sw_tx_rptr        = mp->urdma->reg_base->tx_buf_rptr;
    int                  space =
        (sw_tx_rptr > sw_tx_wptr) ? (sw_tx_rptr - sw_tx_wptr) : (mp->urdma->tx_urdma_size - sw_tx_wptr + sw_tx_rptr);
...
static void URDMA_CheckRxDisable(struct ms_uart_port *mp, u8 *buf, int count)
{
    int       i;
    static u8 urdma_rxdisable  = 0;
    static u8 urdma_paddisable = 0;
    char      isConsole        = (console_port.port.line == mp->port.line) ? 1 : 0;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (5 == urdma_paddisable)
        {
            printk("uart pad disable\n");
...
void URDMA_StartRx(struct uart_port *p)
{
    ...
    U16                  in_size =
        (sw_rx_wptr - sw_rx_rptr) & (mp->urdma->rx_urdma_size - 1); // read from (sw_rx_rptr+1), end at (sw_rx_wptr)
    ...
    if (sw_rx_rptr == mp->urdma->rx_urdma_size - 1) // initial case
    {
        ...
        tty_flip_buffer_push(&p->state->port);
        URDMA_CheckRxDisable(mp, &mp->urdma->rx_buf[0], in_size - rptr_to_end);
    ...
    else if (in_size > rptr_to_end)
    {
        ...
        URDMA_CheckRxDisable(mp, &mp->urdma->rx_buf[sw_rx_rptr + 1], rptr_to_end);
        tty_insert_flip_string(&p->state->port, &mp->urdma->rx_buf[0], in_size - rptr_to_end);
        URDMA_CheckRxDisable(mp, &mp->urdma->rx_buf[0], in_size - rptr_to_end);
        p->icount.rx += in_size;
        // update global sw_rx_rptr
    ...
    {
        tty_insert_flip_string(&p->state->port, &mp->urdma->rx_buf[(sw_rx_rptr + 1) & (mp->urdma->rx_urdma_size - 1)],
                               in_size);
        p->icount.rx += in_size;
        // update global sw_rx_rptr
        mp->urdma->sw_rx_rptr += in_size & (mp->urdma->rx_urdma_size - 1); // avoid sw_rx_rptr overflow
        tty_flip_buffer_push(&p->state->port);
        URDMA_CheckRxDisable(mp, &mp->urdma->rx_buf[(sw_rx_rptr + 1) & (mp->urdma->rx_urdma_size - 1)], in_size);
