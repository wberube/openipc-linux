...
extern int infinity_sha_update(u64 in, u32 len, u32 *state, u32 count, u8 once);
...
static int cipher_aes_crypto(MDRV_AES_HANDLE *handle, AES_DIR dir)
{
    ...
    ret = infinity_aes_crypt_pub(&op, (u64)handle->in_pa, (u64)handle->out_pa);
    if (ret != handle->len)
    {
...
int cipher_sha_update(MDRV_SHA_HANDLE *handle)
{
    ...
    ret = infinity_sha_update(Chip_Phys_to_MIU(handle->u32DataPhy), handle->u32DataLen, handle->ctx.state,
                              handle->ctx.count, once);
    if (ret == 0)
    {
