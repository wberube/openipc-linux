...
static int infinity_sha256_update(struct shash_desc *desc, const u8 *data, unsigned int len)
{
    ...
    while (leftover >= SHA256_BLOCK_SIZE)
    {
        ...
        SHA_DBG("\t\t %s,#%d leftover:%llu infinity_ctx->u32Bufcnt(%d) \n", __FUNCTION__, __LINE__, leftover,
                infinity_ctx->u32Bufcnt);

        memcpy(ALLOC_DMEM.aesdma_vir_SHABuf_addr + infinity_ctx->u32Bufcnt, data + u32InputCopied,
               estimate_size - infinity_ctx->u32Bufcnt);

        if (sctx->count)
        {
            HAL_SHA_Write_InitValue_BE(sctx->state);
...
int infinity_sha_update(u64 in, u32 len, u32 *state, u32 count, u32 once)
{
    u32 loop    = 0;
    u32 msg_cnt = 0;
