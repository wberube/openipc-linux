...
int __init arm_add_memory(u64 start, u64 size)
{
    ...
    if (aligned_start < MSTAR_MIU0_BUS_BASE)
    {
        if (aligned_start + size <= MSTAR_MIU0_BUS_BASE)
        {
        ...
        pr_info("Ignoring memory below MSTAR_MIU0_BUS_BASE: 0x%08llx-0x%08llx\n", aligned_start,
                (u64)MSTAR_MIU0_BUS_BASE);

        size -= MSTAR_MIU0_BUS_BASE - aligned_start;
        aligned_start = MSTAR_MIU0_BUS_BASE;
    }
...
void get_boot_mem_info(BOOT_MEM_INFO type, phys_addr_t *addr, phys_addr_t *len)
{
    ...
    if (bUseDefMMAP == 0)
    {
        switch (type)
        {
            case LINUX_MEM:
#ifdef CONFIG_ARM64
                *addr = MSTAR_MIU0_BUS_BASE;
#else
                *addr = MSTAR_MIU0_BUS_BASE + PHYS_OFFSET - SOC_LOW_PHYS_START;
#endif
                *len = LXmem;
                break;
            case EMAC_MEM:
...
#endif
#ifdef CONFIG_ARM64
    printk("LXmem is 0x%llx PHYS_OFFSET is 0x%llx\n", (u64)LXmem, (u64)MSTAR_MIU0_BUS_BASE);
#else
    printk("LXmem is 0x%llx PHYS_OFFSET is 0x%llx\n", (u64)LXmem,
           (u64)MSTAR_MIU0_BUS_BASE + PHYS_OFFSET - SOC_LOW_PHYS_START);
#endif
