...
LABEL_SEND_CMD:
    ...
    if (u16_reg & BIT_SD_FCIE_ERR_FLAGS)
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "eMMC_%d cmd[%d] WARN: check SD_STS:%X \n", emmc_ip, u8_CmdIdx,
                       u16_reg);
            goto SEND_CMD_RETRY;
        }
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC_%d cmd[%d] ERR: check SD_STS:%X \n", emmc_ip, u8_CmdIdx, u16_reg);
        u32_err = u16_reg;
        goto LABEL_SEND_CMD_ERROR;
    }
    ...
    if ((e_RspType == eMMC_R1) || (e_RspType == eMMC_R1B))
    {
        ...
        if (eMMC_ST_SUCCESS != u32_err)
        {
            ...
            {
                u8_retry_r1++;
                eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "eMMC_%d cmd[%d] WARN: check R1 error: %X \n", emmc_ip,
                           u8_CmdIdx, u32_err);
                goto SEND_CMD_RETRY;
            }
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC_%d cmd[%d] ERR: check R1 error:%X \n", emmc_ip, u8_CmdIdx,
                       u32_err);
            goto LABEL_SEND_CMD_ERROR;
        }
    }
