...
static volatile U32 fcie_int[EMMC_NUM_TOTAL] = {0};
...
irqreturn_t eMMC_FCIE_IRQ(int irq, void *dummy)
{
    ...
    {
        REG_FCIE_CLRBIT(FCIE_MIE_INT_EN(emmc_ip), (BIT_DMA_END | BIT_ERR_STS));

        fcie_int[emmc_ip] = 1;
        wake_up(&fcie_wait);
        return IRQ_HANDLED;
    }
    else if (u16_Events & (BIT_SD_CMD_END | BIT_ERR_STS))
    {
        REG_FCIE_CLRBIT(FCIE_MIE_INT_EN(emmc_ip), BIT_SD_CMD_END | BIT_ERR_STS);
...
U32 eMMC_WaitCompleteIntr(eMMC_IP_EmType emmc_ip, uintptr_t u32_RegAddr, U16 u16_WaitEvent, U32 u32_MicroSec)
{
    ...
    u32_isr_tmp[0] = fcie_int[emmc_ip];
    ...
    //----------------------------------------
    if (wait_event_timeout(fcie_wait, (fcie_int[emmc_ip] == 1), usecs_to_jiffies(u32_MicroSec)) == 0)
    {
#if eMMC_IRQ_DEBUG
        u32_isr_tmp[1] = fcie_int[emmc_ip];
