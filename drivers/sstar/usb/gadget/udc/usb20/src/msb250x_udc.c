...
+--------------------+---+---+-------------------------------------------------
*/
static int msb250x_udc_write_riu(struct msb250x_ep *ep, struct msb250x_request *req, u32 bytes)
{
    u8 ep_num = 0;
...
+--------------------+---+---+-------------------------------------------------
*/
static int msb250x_udc_read_riu(struct msb250x_ep *ep, struct msb250x_request *req, u32 bytes)
{
    u8  ep_num  = 0;
    int is_last = 0;
...
int msb250x_udc_release_autoNAK_cfg(u8 cfg)
{
    ...
    switch (cfg)
    {
        ...
        case 2:
            ...
            ms_writew((ms_readw(MSB250X_OTG0_AUTONAK2_CTRL)
...
void msb250x_udc_enable_autoNAK(u8 ep_num, u8 cfg)
{
    ...
    switch (cfg)
    {
        ...
        case 1:
            ...
            ms_writeb((ms_readb(MSB250X_OTG0_AUTONAK1_CTRL)
...
void msb250x_udc_ok2rcv_for_packets(u8 cfg, u16 pkt_num)
{
    ...
    switch (cfg)
    {
        case 0:
            ...
            break;
        case 1:
            ms_writew((ms_readw(MSB250X_OTG0_AUTONAK1_RX_PKT_CNT) & ~0xE000) | pkt_num,
            ...
            break;
        case 2:
            ms_writew((ms_readw(MSB250X_OTG0_AUTONAK2_RX_PKT_CNT) & ~0xE000) | pkt_num,
...
void msb250x_udc_allowAck(u8 cfg)
{
    ...
    switch (cfg)
    {
        case 0:
            ...
            ms_writew((ms_readw(MSB250X_OTG0_AUTONAK0_CTRL) | MSB250X_OTG0_AUTONAK0_AllowAck),
                      MSB250X_OTG0_AUTONAK0_CTRL);
            break;
        case 1:
        ...
        case 2:
            ...
            ms_writeb((ms_readw(MSB250X_OTG0_AUTONAK2_CTRL) | MSB250X_OTG0_AUTONAK2_AllowAck),
                      MSB250X_OTG0_AUTONAK2_CTRL);
            break;
    }
                    ...
                    if (1 == csr_l->bRxPktRdy)
                    {
                        ...
                        if (bytes < counts)
                        {
                            printk(KERN_WARNING
                                   ...
                                   ep_num, counts, req->req.buf, req->req.actual, req->req.length);
                        }
...
static irqreturn_t msb250x_udc_link_isr(struct msb250x_udc *dev)
{
    ...
    if (1 == usb_st->bReset)
    {
        ...
        // ms_writeb(0x01, (GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17) + 1));  //HS_TX common mode current enable (100mV)=>
        // bit <7> set 1
#endif
...
static irqreturn_t msb250x_udc_isr(int irq, void *_dev)
{
    ...
    /*
        ms_writeb(ms_readb(GET_REG8_ADDR(REG_ADDR_BASE_PM_GPIO, 0X94)) & ~BIT1, GET_REG8_ADDR(REG_ADDR_BASE_PM_GPIO,
       0X94)); ms_writeb(ms_readb(GET_REG8_ADDR(REG_ADDR_BASE_PM_GPIO, 0X94)) | BIT1,
...
+--------------------+---+---+-------------------------------------------------
*/
int msb250x_udc_get_status(struct msb250x_udc *dev, struct usb_ctrlrequest *crq)
{
    u8 status = 0;
...
static void msb250x_udc_init_MIU_sel(void)
{
    ...
    ms_writeb((ms_readb(GET_REG16_ADDR(USBC_BASE_ADDR, 0x0C) + 1) | 0x1),
...
void msb250x_udc_deinit_utmi(void)
{
    ...
    ms_writeb(0x00, MSB250X_OTG0_PWR_REG);
    ms_writeb(0x40, GET_REG16_ADDR(USBC_BASE_ADDR, 1));
    ...
    ctrl_l                                = ms_readb(GET_REG16_ADDR(USBC_BASE_ADDR, 0));
    pst_usbc0_rst_ctrl_l->bOTG_RST        = 0;
    pst_usbc0_rst_ctrl_l->bOTG_XIU_ENABLE = 0;
    ms_writeb(ctrl_l, GET_REG16_ADDR(USBC_BASE_ADDR, 0));

    ms_writew(ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x03)) | BIT0 | BIT1 | BIT8,
              GET_REG16_ADDR(UTMI_BASE_ADDR, 0x03));
...
{
    U16 u16_OTP_USB_XCVR_P0;
    U16 u16_GCR_HS_TRIM_CODE;
    U16 u16_GCR_HS_SW_TRIM_CODE;
    ...
    ms_writeb(((ms_readb(GET_REG16_ADDR(USBC_BASE_ADDR, 0x01)) & ~(BIT0 | BIT1)) | BIT1),
              GET_REG16_ADDR(USBC_BASE_ADDR, 0x01));
#endif
    ...
    ms_writew(0x6BC3,
              GET_REG16_ADDR(UTMI_BASE_ADDR, 0)); // Turn on UPLL, reg_pdn: bit<9> reg_pdn: bit<15>, bit <2> ref_pdn
    mdelay(1);
     ...
     */
    ms_writew(0x7F05, GET_REG16_ADDR(UTMI2_BASE_ADDR, 0)); // UTMI2
#endif
#endif

    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x3c)) | 0x01),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x3c)); // set CA_START as 1
    mdelay(10);
    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x3c)) & ~0x01),
              GET_REG16_ADDR(UTMI_BASE_ADDR, 0x3c)); // release CA_START
    while (0 == (ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x3c)) & 0x02))
        ; // polling bit <1> (CA_END)

    // msb250x_udc_init_usb_ctrl();

    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x06)) & 0x9F) | 0x40,
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x06)); // reg_tx_force_hs_current_enable
    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x03)) | 0x28),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x03)); // Disconnect window select
    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x03)) & 0xef),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x03)); // Disconnect window select
    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x07)) & 0xfd),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x07)); // Disable improved CDR

    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x09)) | 0x81),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x09)); // UTMI RX anti-dead-loc, ISI effect improvement
    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x15)) | 0x20),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x15)); // Chirp signal source select
    ms_writeb((ms_readb(GET_REG8_ADDR(UTMI_BASE_ADDR, 0x0b)) | 0x80),
              GET_REG8_ADDR(UTMI_BASE_ADDR, 0x0b)); // set reg_ck_inv_reserved[6] to solve timing problem

#if 0 // defined(CONFIG_SSTAR_CEDRIC)
    ...
    ms_writeb((ms_readb(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x02)) | 0x80),
              GET_REG16_ADDR(UTMI_BASE_ADDR, 0x02)); // avoid glitch
#endif
    /* USB2 PHY add new Trim value : Read from OTP and write into UTMI */
    u16_OTP_USB_XCVR_P0     = ms_readw(GET_REG16_ADDR(REG_ADDR_BASE_OTP, 0x54)); // Read 0x101F_54
    u16_GCR_HS_TRIM_CODE    = u16_OTP_USB_XCVR_P0 & 0x003F;
    u16_GCR_HS_SW_TRIM_CODE = (u16_OTP_USB_XCVR_P0 >> 2) & 0x03F0;

    if (u16_OTP_USB_XCVR_P0 & BIT12)
    {
        ms_writew(((ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x45)) & (~(0x003F))) | u16_GCR_HS_TRIM_CODE),
                  GET_REG16_ADDR(UTMI_BASE_ADDR, 0x45));
        ms_writew(((ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x44)) & (~(0x03F0))) | u16_GCR_HS_SW_TRIM_CODE),
                  GET_REG16_ADDR(UTMI_BASE_ADDR, 0x44));
    }

    tx_swing_set(tx_swing);
    de_emphasis_current_set(dem_cur);
    cm_current_set(cm_cur);
}
...
void msb250x_udc_reset_otg(void)
{
    ...
    ms_writeb(ms_readb(MSB250X_OTG0_USB_CFG0_L) | MSB250X_OTG0_CFG0_SRST_N, MSB250X_OTG0_USB_CFG0_L);  // default set as
                                                                                                       // 1
}
...
void msb250x_udc_init_otg(struct msb250x_udc *udc)
{
    ...
    // ms_writeb((ms_readb(MSB250X_OTG0_USB_CFG6_H) | (MSB250X_OTG0_CFG6_H_SHORT_MODE | MSB250X_OTG0_CFG6_H_BUS_OP_FIX |
    // MSB250X_OTG0_CFG6_H_REG_MI_WDFIFO_CTRL)), MSB250X_OTG0_USB_CFG6_H);
...
static ssize_t de_emphasis_current_store(struct device *dev, struct device_attribute *attr, const char *buf,
                                         size_t count)
{
    u8 value;
...
static int msb250x_udc_probe(struct platform_device *pdev)
{
    ...
    if (of_property_read_u32_array(pdev->dev.of_node, "tx-swing,de-emphasis,cm-current", val, ARRAY_SIZE(val)))
    {
        printk(KERN_DEBUG "<USB>[GADGET] read property fail\n");
    }
...
static struct of_device_id sstar_udc_of_device_ids[] = {
    {.compatible = "sstar,infinity-udc"},
    {},
};

static struct platform_driver ss_udc_driver = {.probe  = msb250x_udc_probe,
...
#endif
                                               .driver = {
                                                   .name = "soc:Sstar-udc", .of_match_table = sstar_udc_of_device_ids,
