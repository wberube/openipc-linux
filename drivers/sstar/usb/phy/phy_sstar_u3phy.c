...
extern int sstar_utmi_port_remove(struct sstar_phy_port *phy_port);
extern int sstar_pipe_port_remove(struct sstar_phy_port *phy_port);
...
static int sstar_u3phy_port_remove(struct device *dev, struct sstar_phy_port *u3phy_port, struct device_node *np)
{
    int ret;
    ...
    if (!of_prop_cmp("pipe", np->name))
    {
        ret = sstar_pipe_port_remove(u3phy_port);
    }
    else
    {
        ret = sstar_utmi_port_remove(u3phy_port);
    }

    devm_phy_put(dev, u3phy_port->phy);
    return ret;
}
...
static int sstar_u3phy_remove(struct platform_device *pdev)
{
    ...
    for_each_available_child_of_node(np, child_np)
    {
        ...
        ret = sstar_u3phy_port_remove(dev, u3phy_port, child_np);
