...
void Hal_CARD_IPOnceSetting(IpOrder eIP)
{
    ...
    else if (eIpSel == IP_SDIO)
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_FCIE),
                        BIT06_T | BIT05_T | BIT04_T | BIT03_T | BIT02_T | BIT01_T | BIT00_T);
...
void Hal_CARD_ConfigSdPad(IpOrder eIP, PadOrder ePAD) // Hal_CARD_SetPADToPortPath
{
    ...
    if (0 != mdrv_padmux_active())
    {
        ...
        MDrv_GPIO_PadVal_Set(gu32_SdCDZ[eIpSel], gu32_SdCdzMode[eIpSel]);
    }
    UNUSED(ePadSel);
#else //
    ...
    if (eIpSel == IP_SD)
    {
        if (ePadSel == PAD_SD_MD1)
        {
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT12_T | BIT11_T | BIT10_T | BIT09_T | BIT08_T);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x68), BIT13_T | BIT12_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67),
                            BIT08_T | BIT11_T); //[B8:B10]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
        else if (ePadSel == PAD_SD_MD2)
        {
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x60), 0x0000);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT12_T | BIT11_T | BIT10_T | BIT09_T | BIT08_T);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x68), BIT13_T | BIT12_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67),
                            BIT09_T | BIT12_T); //[B8:B10]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
        else if (ePadSel == PAD_SD_MD3)
        {
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x60), 0x0000);
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x68), BIT13_T | BIT12_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67),
                            BIT08_T | BIT09_T | BIT12_T); //[B8:B10]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
        else if (ePadSel == PAD_SD_MD4)
        {
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x60), 0x0000);
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x68), BIT13_T | BIT12_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67),
                            BIT10_T | BIT11_T | BIT12_T); //[B8:B10]/[reg_sd0_mode:reg_sd0_cdz_mode]
        }
    }
    else if (eIpSel == IP_SDIO)
    {
        if (ePadSel == PAD_SD_MD2)
        {
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x61), BIT09_T | BIT08_T | BIT04_T | BIT00_T);
    ...
    if (eIpSel == IP_SDIO)
    {
        if (ePadSel == PAD_SD_MD2)
        {
            if (ePinPull == EV_PULLDOWN)
            {
                ...
                REG_FCIE_SETBIT(GET_CARD_REG_ADDR(PAD_CHIPTOP_BASE, 0x3a),
            ...
            else if (ePinPull == EV_PULLUP)
            {
                ...
                // need setting this bit to ensure GPIO mode bit0~5:clk cmd d0~3
                CARD_REG_CLRBIT(GET_CARD_REG_ADDR(PAD_CHIPTOP_BASE, 0x3a),
                                BIT00_T | BIT01_T | BIT02_T | BIT03_T | BIT04_T | BIT05_T);
...
void Hal_CARD_DrvCtrlPin(IpOrder eIP, PadOrder ePAD)
{
    ...
    U8_T u8_PadClk, u8_ClkDrvMax      = DRV_CTRL_1;
    U8_T u8_PadCmd, u8_CmdDrvMax      = DRV_CTRL_1;
    U8_T u8_PadData[4], u8_DataDrvMax = DRV_CTRL_1, i;

#if (PADMUX_SET == PADMUX_SET_BY_FUNC)
    u8_PadClk = gu32_SdCLK[eIpSel];
    u8_PadCmd = gu32_SdCMD[eIpSel];
    for (i = 0; i < 4; i++)
        u8_PadData[i] = gu32_SdDAT[eIpSel][i];
...
void Hal_CARD_SetClock(IpOrder eIP, U32_T u32ClkFromIPSet)
{
    ...
    else if (eIpSel == IP_SDIO)
    {
        ...
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_FCIE), BIT06_T); // boot sel
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_SC_GP_CTRL_BANK, 0x25),
                        BIT05_T); //[B3/B7]:[SDIO/SD] select BOOT clock source (glitch free) - 0: select BOOT clock
                                  // 12MHz (xtali), 1: select FCIE/SPI clock source
...
void Hal_CARD_PowerOn(IpOrder eIP, U16_T u16DelayMs)
{
    ...
    switch (gu32_SdPWR[eIP])
    {
        case PAD_PM_GPIO9:
            ...
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT01_T);
            break;
        case PAD_FUART_RTS:
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT03_T);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT02_T);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT01_T);
            break;
...
void Hal_CARD_PowerOff(IpOrder eIP, U16_T u16DelayMs)
{
    ...
    switch (gu32_SdPWR[eIP])
    {
        case PAD_PM_GPIO9:
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT03_T);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT02_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x22), BIT01_T);
            break;
        case PAD_FUART_RTS:
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT03_T);
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT02_T);
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x34), BIT01_T);
            break;
...
// PADMUX
    ...
    if (0 != mdrv_padmux_active())
    {
        MDrv_GPIO_PadVal_Set(gu32_SdCDZ[eIP], gu32_SdCdzMode[eIP]);
    }

    // GPIO
    // Whatever mdrv_padmux_active is ON or OFF, just set it to input mode.
    MDrv_GPIO_Pad_Set(gu32_SdCDZ[eIP]);
...
BOOL_T Hal_CARD_GetCdzState(IpOrder eIP) // Hal_CARD_GetGPIOState
{
    ...
    if (MDrv_GPIO_Pad_Read((gu32_SdCDZ[eIP]), &nLv) != 0)
    {
        pr_sd_err("sdmmc error ! [%s][%d]\n", __FUNCTION__, __LINE__);
        goto fail;
    }
